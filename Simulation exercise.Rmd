...
ggplot(data = exponentialDistributionMeans, aes(x = means)) + 
              ...
              labs(title = "Distribution of averages of samples,
...
**Infer:** As you can see standard deviation and variance in theory (`r sd` and `r Var`) and sample (`r sd_x` and `r Var_x`) are pretty close.
...
**Infer:** As shown in the graph, the calculated distribution of means of random sampled exponantial distributions overlaps with the normal distribution, due to the Central Limit Theorem. The more samples we would get (now 1000), the closer will the density distribution be to the normal distribution bell curve.
